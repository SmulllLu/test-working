# January 2026

### Pull #5 [ruyisdk-test/ruyisdk-eclipse-plugins-test/pull#5](https://github.com/ruyisdk-test/ruyisdk-eclipse-plugins-test/pull/5)
- 对虚拟环境进行了更新
    - 自动检测工作区变化
- 修复了[#82](https://github.com/ruyisdk/ruyisdk-eclipse-plugins/issues/82#issuecomment-3837909308)中第一个问题：安装ruyi时按钮未被disable

### Issue #107 [ruyisdk/ruyisdk-eclipse-plugins/issues#107](https://github.com/ruyisdk/ruyisdk-eclipse-plugins/issues/107)
- 反馈了通过Eclipse Marketplace安装了插件v0.1.2，提示框仍然可以任意关闭，且关闭提示框后进程仍在后台继续下载的问题。

### Doc
- 使用qemu的OpenEuler构建项目
	- OpenTTD:[构建文档](https://github.com/ruyisdk-test/test-working/tree/main/YXCZS/blog/OpenTTD.md)
	- Yolo_Label:[构建文档](https://github.com/ruyisdk-test/test-working/tree/main/YXCZS/blog/Yolo_Label.md)
	- supertux:[构建文档](https://github.com/ruyisdk-test/test-working/tree/main/YXCZS/blog/supertux.md)
